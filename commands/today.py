...
class today(BaseCommand):
    ...
    async def handle(self, params, message, client):
        ...
        embed.add_field(name="🏥เคสสะสม (ยกเว้นเดินทางจากต่างประเทศ)", value=f"{ans[0]['total_case_excludeabroad']:,}", inline=False)
        embed.add_field(name="🪦เสียชีวิต", value=f"{ans[0]['new_death']:,}", inline=False)
        embed.add_field(name="🪦เสียชีวิตสะสม", value=f"{ans[0]['total_death']:,}", inline=False)
        embed.add_field(name="🌼หายป่วยวันนี้", value=f"{ans[0]['new_recovered']:,}", inline=False)
        embed.add_field(name="🌼หายป่วยสะสม", value=f"{ans[0]['total_recovered']:,}", inline=False)
        embed.add_field(name="📅ข้อมูลอัปเดตล่าสุด", value=ans[0]['update_date'], inline=False)
