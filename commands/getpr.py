    ...
    async def handle(self, params, message, client):
        ...
        if exist:
            ...
            embed = nextcord.Embed(
                title="สถานการณ์ COVID-19 จังหวัด"+ans[i]['province'])
            embed.add_field(name="🚑เคสใหม่",
                            value=f"{ans[i]['new_case']:,}", inline=False)
            embed.add_field(name="🚑เคสใหม่ (ยกเว้นเดินทางจากต่างประเทศ)",
                            value=f"{ans[i]['new_case_excludeabroad']:,}", inline=False)
            embed.add_field(name="🏥เคสสะสม",
                            value=f"{ans[i]['total_case']:,}", inline=False)
            embed.add_field(name="🏥เคสสะสม (ยกเว้นเดินทางจากต่างประเทศ)",
                            value=f"{ans[i]['total_case_excludeabroad']:,}", inline=False)
            embed.add_field(name="🪦เสียชีวิต",
                            value=f"{ans[i]['new_death']:,}", inline=False)
            embed.add_field(name="🪦เสียชีวิตสะสม",
                            value=f"{ans[i]['total_death']:,}", inline=False)
